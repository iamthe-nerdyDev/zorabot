...
// File: ZorlifyPricePredictions.sol
    ...
    ReentrancyGuard
{
    ...
    function fulfillRequest(
        ...
        if (response.length > 0) {
            ...
            if (market.targetIsAboveTargetPrice) {
                market.outcome = price > market.targetPrice
                    ? MarketOutcome.YES
                    : MarketOutcome.NO;
            } else {
                market.outcome = price < market.targetPrice
    ...
    function claim(
        ...
        Market storage market = markets[marketId];
        if (!market.resolved || market.outcome == MarketOutcome.UNRESOLVED)
        ...
        returns (uint256 yesShares, uint256 noShares, bool hasClaimed)
    {
        Market storage market = markets[marketId];
        return (
            market.yesSharesBalance[usr],
            market.noSharesBalance[usr],
            market.hasClaimed[usr]
        );
    }
